...
export default function Food() {
  ...
  const filtereddata = data.filter((list) => list.name === "Business");

  return (
    <div>
      <div>
        <h1 className="maintext">Business</h1>
        <hr className="horizon1"></hr>
      </div>

      <div className="containermain">
        {filtereddata
          .filter((value) => (Update ? value.id >= 35 : value.id <= 37))
          .map((items) => {
            return (
              <div
                className="newtab"
