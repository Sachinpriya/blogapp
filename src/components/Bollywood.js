...
export default function Bollywood() {
  ...
  const filtereddata = data.filter((list) => list.name === "Bollywood");

  return (
    <div>
      <div>
        <h1 className="maintext">Bollywood</h1>
        <hr className="horizon1"></hr>
      </div>

      <div className="containermain">
        {filtereddata
          .filter((value) => (Update ? value.id >= 24 : value.id <= 27))
          .map((items) => {
            return (
              <div
                className="newtab"
