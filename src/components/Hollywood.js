...
export default function Hollywood() {
  ...
  const filtereddata = data.filter((list) => list.name === "Hollywood");

  return (
    <div>
      <div>
        <h1 className="maintext">Hollywood</h1>
        <hr className="horizon1"></hr>
      </div>

      <div className="containermain">
        {filtereddata
          .filter((value) => (Update ? value.id >= 17 : value.id <= 19))
          .map((items) => {
            return (
              <div
                className="newtab"
