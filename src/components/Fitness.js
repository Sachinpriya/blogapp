...
export default function Fitness() {
  ...
  const filtereddata = data.filter((list) => list.name === "Health");

  return (
    <div>
      <div>
        <h1 className="maintext">Fitness</h1>
        <hr className="horizon1"></hr>
      </div>

      <div className="containermain">
        {filtereddata
          .filter((value) => (Update ? value.id >= 44 : value.id <= 46))
          .map((items) => {
            return (
              <div
                className="newtab"
